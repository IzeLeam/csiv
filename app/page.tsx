...
export default function Home() {
  ...
  const frequencyDisplay = (value: string) => {
    ...
    if (value.toLowerCase() === "medium")
      return t(language, "frequencyMedium");
        ...
        <main className="flex flex-1 flex-col gap-6">
          ...
          >
              <div className="mb-4 flex items-center justify-between text-xs text-zinc-400">
                <span>
                  {t(language, "cardCategory")}: {categoryDisplay(
                    currentQuestion!.category
                  )}
                  {" · "}
                  {t(language, "cardDifficulty")}: {difficultyDisplay(
                    currentQuestion!.difficulty
                  )}
                  {" · "}
                  {t(language, "cardFrequency")}: {frequencyDisplay(
